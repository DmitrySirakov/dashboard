...
st.set_page_config(
    page_title="Цифровая кафедра", page_icon=":bar_chart:", layout="wide"
)
...
def get_data_from_cloud(n_list=0):
    ...
    docid = "1NQrxXH0BoT4QsMsTzKqbJEmz65f7DopPtbyAZYJp4j0"
    ...
    alphabet_competetions = [
        "Применяет языки программирования для решения профессиональных задач",
    ...
    df["Результат"] = (
        df["Результат"].apply(lambda x: str(x)[:4]).replace("-", 0).astype(int)
        / 10**4
    )
    df["Институт"] = df["Институт"].astype(str)
    test_ass = ["Т_ДСК", "Т_ДЗЗ", "Т_СКТ"]
    df = df.query(
        "`Наименование курса` != @test_ass[0] & `Наименование курса` != @test_ass[1] & `Наименование курса` != @test_ass[2]"
    )
    ...
    for i in list_of_fio:
        if (
            "Завершено"
            in df.query("`Этап ассесмента` == 3 & Слушатель == @i")["Статус"].unique()
        ):
            df.loc[
                (df["Этап ассесмента"] == 3) & (df["Слушатель"] == i), "Статус"
            ] = "Завершено"
        elif (
            "Старт"
            in df.query("`Этап ассесмента` == 3 & Слушатель == @i")["Статус"].unique()
...
authenticator = stauth.Authenticate(
    credentials, "IT-center_dashboard", "abcdef", cookie_expiry_days=30
)
...
if authentication_status:
    ...
    df_selection = (
        df[
            [
                ...
                "ДПП группа",
                "Телефон",
                "Кол-во академических задолженностей",
                "Внутренний прокторинг",
                "Наименование компетенции",
                "Результат",
            ]
        ]
        .query(
            "Институт == @institute & Курс == @course & `Этап ассесмента` == @assessment & `Наименование курса` == @programms & `Уровень обучения` == @level & Статус == @final_assessment & `Внутренний прокторинг`==@proctor"
        )
        .drop_duplicates()
        .reset_index()
        .drop(columns=["index"])
    )  # & Наименование курса == @programms & Курс == @course
    ...
    people_by_institute = (
        df_selection[["Институт", "Слушатель"]]
        .drop_duplicates()
        .groupby("Институт")
        .count()["Слушатель"]
    )
    ...
    people_by_programms = (
        df_selection[["Слушатель", "Наименование курса", "Этап ассесмента", "Статус"]]
        .query("`Этап ассесмента` == @assessment & Статус == @final_assessment")
        .drop(columns=["Этап ассесмента", "Статус"])
        .drop_duplicates(subset=["Слушатель"])
        .groupby("Наименование курса")
        .agg({"Наименование курса": "count"})
    )
    ...
    df_3 = (
        df_selection.query(
            "`Этап ассесмента` == 3 & Статус == @status & `Наименование курса` == @programms"
        )[["Слушатель", "Наименование компетенции", "Результат", "Этап ассесмента"]]
        .drop_duplicates()
        .groupby(["Наименование компетенции", "Этап ассесмента"], as_index=False)
        .agg({"Результат": "mean"})
    )
    df_2 = (
        df_selection.query(
            "`Этап ассесмента` == 2 & Статус == @status  & `Наименование курса` == @programms"
        )[["Слушатель", "Наименование компетенции", "Результат", "Этап ассесмента"]]
        .drop_duplicates()
        .groupby(["Наименование компетенции", "Этап ассесмента"], as_index=False)
        .agg({"Результат": "mean"})
    ...
    df_1 = (
        df_selection.query(
            "`Этап ассесмента` == 1 & Статус == @status & `Наименование курса` == @programms"
        )[["Слушатель", "Наименование компетенции", "Результат", "Этап ассесмента"]]
        .drop_duplicates()
        .groupby(["Наименование компетенции", "Этап ассесмента"], as_index=False)
        .agg({"Результат": "mean"})
    ...
    df_competitions = df_competitions[df_competitions["Результат"] > 0.2]
    fig = px.line_polar(
        df_competitions,
        r="Результат",
    ...
    if text_input:
        st.dataframe(
            df_selection[
                df_selection["Слушатель"].apply(lambda x: x.split()[0]) == text_input
            ]
        )
    else:
        st.dataframe(df_selection)

    st.download_button(label="📥 Сделать выгрузку", data=df_xlsx, file_name="База.xlsx")
    # ----------------     HideStreamlitStyle     ----------------#
    hide_st_style = """
            <style>
