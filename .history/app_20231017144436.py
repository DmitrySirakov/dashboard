...
if authentication_status:
    ...
    df_3 = (
        df.query("`Этап ассесмента` == 3 & Статус == @status")[
    ...
    df_2 = (
        df.query("`Этап ассесмента` == 2 & Статус == @status")[
            ["Слушатель", "Наименование компетенции", "Результат", "Этап ассесмента"]
        ]
        .drop_duplicates()
        .groupby(["Наименование компетенции", "Этап ассесмента"], as_index=False)
    ...
    df_1 = (
        df.query("`Этап ассесмента` == 1 & Статус == @status")[
            ["Слушатель", "Наименование компетенции", "Результат", "Этап ассесмента"]
        ]
        .drop_duplicates()
        .groupby(["Наименование компетенции", "Этап ассесмента"], as_index=False)
    ...
    st.download_button(label="📥 Сделать выгрузк", data=df_xlsx, file_name="База.xlsx")
    # ----------------     HideStreamlitStyle     ----------------#
    hide_st_style = """
            <style>
